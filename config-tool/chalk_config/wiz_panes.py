...
def deal_with_overwrite_widget():
    ...
    if v != "":
        arr = cursor.execute("SELECT id from configs where name=?", [v]).fetchone()
        if arr != None:
            old_id = arr[0]
...
class ApiAuth(WizContainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.crashoverride_auth_obj = crash_override_authn.CrashOverrideAuth(
            co_auth_url
        )
    ...
    def authstatuscheck(self):
        ...
        if self.is_authenticated() or self.has_failed():
            logger.info(
                "Bounced from authentication status check, already authenticated or already"
            )
            return None
        ...
        try:
            logger.debug(
                "Checking authentication status at %s"
                % (self.crashoverride_auth_obj.poll_url)
            )
            ...
            logger.error(
                ...
                % (self.crashoverride_auth_obj.poll_url, err)
            )

            # Token verification error - this is bad, login failed - post a message
            self.post_message(
                self.AuthSuccess(
                    self.crashoverride_auth_obj, "id_token_verification_failure"
                )
            )
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                self.crashoverride_auth_obj.decoded_jwt = (
                    self.crashoverride_auth_obj._decode_and_validate_jwt(
                        resp_json["token"]
                    )
                )
                self.crashoverride_auth_obj.revision_id = (
                    self.crashoverride_auth_obj.decoded_jwt["revisionId"]
                )
                self.crashoverride_auth_obj.token_issued_at = (
                    self.crashoverride_auth_obj.decoded_jwt["iat"]
                )
                logger.info(
                    "Decoded JWT: %s" % (self.crashoverride_auth_obj.decoded_jwt)
                )
                ...
                self.crashoverride_auth_obj.user_id = (
                    self.crashoverride_auth_obj._user_json["id"]
                )
                self.crashoverride_auth_obj.user_name = (
                    self.crashoverride_auth_obj._user_json["name"]
                )
                self.crashoverride_auth_obj.user_email = (
                    self.crashoverride_auth_obj._user_json["email"]
                )
                self.crashoverride_auth_obj.user_picture = (
                    self.crashoverride_auth_obj._user_json["picture"]
                )
                logger.info(
                    "User %s (%s) logged in"
                    % (
                ...
                self.post_message(
                    self.AuthSuccess(self.crashoverride_auth_obj, "success")
                )
                ...
                logger.debug(
                    "HTTP 428 received, authentication not succeeded yet. Sleeping until next interval......."
                )
                return None
                ...
                logger.error(
                    "Unknown HTTP code returned. Expecting on 428's and 200's ? %d"
                    % (resp.status_code)
                )
                return None
    ...
    def start_auth_polling(self):
        ...
        if not self.chalk_code_json:
            logger.error(
                "Error getting Chalk device code, no JSON returned by _get_code()"
            )
            return -1
        ...
        self.auth_widget_2.update(
            "Or, browse to\n\n1. %s\n\nAlternatively \n\n1. Navigate to: %s\n2. Enter the following code: %s\n\nIf you would like to login from another device, click the button to display a QR code"
            % (
                self.chalk_code_json["authUrl"],
                self.chalk_code_json["authUrl"].split("?")[0],
                self.chalk_code_json["id"],
            )
        )
        self.auth_widget_2.refresh()

        # Start background task that polls oauth
        self.auth_status_checker = self.set_interval(
            self.crashoverride_auth_obj.poll_int, self.authstatuscheck
        )
        logger.info(
            "Background authN checker begun with interval of %d seconds"
            % (self.crashoverride_auth_obj.poll_int)
        )
        return 0
...
class BuildBinary(WizContainer):
    def compose(self):
        ...
        yield Container(
            ...
            RadioSet(
                RadioButton(RELEASE_BUILD, True, id="release_build"),
                RadioButton(DEBUG_BUILD, id="debug_build"),
                id="set_bin_debug",
            ),
            Horizontal(
                Input(placeholder=PLACEHOLD_EXE, id="exe_name"),
                Label(L_BIN_NAME, classes="label"),
            ),
            Horizontal(
                Input(placeholder=PLACEHOLD_CONF, id="conf_name"),
                Label(L_CONF_NAME, classes="label"),
            ),
            Label(L_NOTE, id="note_label"),
            Input(placeholder=PLACEHOLD_NOTE, id="note"),
            Horizontal(
                Switch(id="overwrite_config", value=False),
...
class ChalkOpts(WizContainer):
    def compose(self):
        ...
        yield RadioSet(
            RadioButton(R_CMIN, value=True, id="chalk_minimal"),
...
class ReportingOptsChalkTime(WizContainer):
    def compose(self):
        ...
        yield RadioSet(
            RadioButton(R_RMIN, id="crpt_minimal"),
        ...
        yield ReportingContainer(
            ...
            EnablingCheckbox("redaction", CR_REDACT, disabled=True, id="crpt_env"),
...
class LogParams(WizContainer):
    def compose(self):
        ...
        yield Horizontal(
            Label(L_LOG_LOC, classes="label"),
            Input(placeholder="/path/to/log/file", id="log_loc"),
        )
        yield Horizontal(Switch(id="log_truncate"), Label(L_LOG_SIZE, classes="label"))
...
class S3Params(WizContainer):
    def compose(self):
        ...
        yield Grid(
            Label("s3://", classes="label emphLabel"),
...
class ReportingPane(WizContainer):
    inline_login_btn = LoginButton(
        label="Login", classes="basicbutton", id="wiz_login_button"
    )
    ...
    def compose(self):
        ...
        yield Horizontal(
            C0ApiToggle(value=True, id="report_co"),
            Label(CO_CRASH, classes="label"),
            self.inline_login_btn,
        )
        yield Horizontal(
            Switch(value=True, id="report_stdout"), Label(CO_STDOUT, classes="label")
        )
        yield Horizontal(
            Switch(value=True, id="report_stderr"), Label(CO_STDERR, classes="label")
        )
        yield Horizontal(
            EnablingSwitch("log_conf", CO_LOG, id="report_log"),
            Label(CO_LOG, classes="label"),
        )
        yield Horizontal(
            EnablingSwitch("http_conf", CO_POST, id="report_http"),
            Label(CO_POST, classes="label"),
        )
        yield Horizontal(
            EnablingSwitch("s3_conf", CO_S3, id="report_s3"),
        ...
        yield Horizontal(
            Switch(value=False, id="env_adds_report"),
            Label(L_ADD_REPORT, classes="label"),
        )
        yield Horizontal(
            EnvToggle(value=False, id="env_custom"),
