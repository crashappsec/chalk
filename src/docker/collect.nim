...
let dockerImageAutoMap: JsonToChalkKeysMapping = {
  ...
  "Config:Digest":                              ("_IMAGE_CONFIG_DIGEST", JsonTransformer((x: JsonNode) =>
                                                    `%`(x.getStr().extractDockerHash()))),
  "Descriptor:annotations:\"config.digest\"":   ("_IMAGE_CONFIG_DIGEST", JsonTransformer((x: JsonNode) =>
                                                    `%`(x.getStr().extractDockerHash()))),
  }.toOrderedTable()
...
proc collectCommon(chalk: ChalkObj, contents: JsonNode, map = dockerImageAutoMap) =
  if not isDockerOverlayFS():
    chalk.setIfNeeded("_IMAGE_CONFIG_DIGEST", chalk.imageId)
...
proc collectImageFrom(chalk:    ChalkObj,
  ...
  let
    ...
    repotags           = parseImages(caseless{"repotags"}.getStrElems(), defaultTag = "")
    repodigests        = parseImages(caseless{"repodigests"}.getStrElems(), defaultTag = "")
    repos              = (repotags & repodigests & repos).asSomeImageRepos()
  ...
  chalk.setIfNeeded("_IMAGE_CONFIG_DIGEST",   configDigest.extractDockerHash())
  chalk.setIfNeeded("DOCKER_PLATFORM",        $platform.normalize())
  chalk.repos          = newOrderedTable[string, DockerImageRepo]()
  var
    collConfigDigest   = unpack[string](chalk.collectedData.getOrDefault("_IMAGE_CONFIG_DIGEST", pack("")))
    repoFoundManifests = newTable[DockerImageRepo, seq[DockerManifest]]()
  for _, repo in repos:
    discard repoFoundManifests.mgetOrPut(repo, newSeq[DockerManifest]())
    # fetch by any known digest
    for i in repo.manifests:
      try:
        let manifest = fetchImageManifest(i, platform)
        repoFoundManifests[repo].add(manifest)
        # if the config digest is unknown such as due to docker inspect not reporting
        # it but the registry digest is known, then use the config digest from registry
        if collConfigDigest == "":
          collConfigDigest = manifest.config.digest.extractDockerHash()
          chalk.setIfNeeded("_IMAGE_CONFIG_DIGEST", collConfigDigest)
      except:
        trace("docker: " & getCurrentExceptionMsg())
  for repo, foundManifests in repoFoundManifests:
    # in docker overlayfs with both --push --load,
    # the loaded digest of the image might not match the pushed digest
    # as the media types might not match
    # in which case we match the image manifest by the config digest,
    # if its known as that should be immutable after the build.
    # also note this backfill is in a different loop to allow to populate
    # config digest from another registry where perhaps its digest is already known
    if len(foundManifests) == 0 and collConfigDigest != "" and isDockerOverlayFS():
      trace("docker: could not manifests by digest. attempting to match tags by config")
      for i in repo.tagImages:
        try:
          let manifest = fetchImageManifest(i, platform)
          if manifest.config.digest.extractDockerHash() == collConfigDigest:
            repoFoundManifests[repo].add(manifest)
        except:
          trace("docker: " & getCurrentExceptionMsg())
  for repo, foundManifests in repoFoundManifests:
    if len(foundManifests) == 0:
      continue
    for i in foundManifests:
      annotations.update(i.annotations)
      let imageRepo = i.asImageRepo(tags = repo.tags.toSeq())
      ...
      let url = annotations{"org.opencontainers.image.url"}.getStr(i.name.url())
      if url != "":
        discard repoUrls.hasKeyOrPut(i.name.registry, newOrderedTable[string, string]())
        discard repoUrls[i.name.registry].hasKeyOrPut(i.name.name, url)
...
proc collectImageManifest*(chalk: ChalkObj,
  ...
  chalk.collectImageFrom(contents, $name, repos = allrepos, configDigest = manifest.config.digest)
  chalk.collectProvenance()
  chalk.collectSBOM()
