...
proc isAwsEc2Host(vendor: string): bool =
  ...
  # older Xen instances
  let uuid = tryToLoadFile(chalkConfig.get[:string]("cloud_provider.cloud_instance_hw_identifiers.sys_hypervisor_path"))
  ...
  let product_uuid = tryToLoadFile(chalkConfig.get[:string]("cloud_provider.cloud_instance_hw_identifiers.sys_product_path"))
...
proc cloudMetadataGetrunTimeHostInfo*(self: Plugin, objs: seq[ChalkObj]):
                               ChalkDict {.cdecl.} =
  result = ChalkDict()
  let vendor = tryToLoadFile(chalkConfig.get[:string]("cloud_provider.cloud_instance_hw_identifiers.sys_vendor_path"))
