...
proc regularTerminationSignal(signal: cint) {.noconv.} =
  ...
  try:
    ...
    if chalkConfig.get[:bool]("chalk_debug"):
...
proc reportTmpFileExitState*(files, dirs, errs: seq[string]) =
  ...
  if chalkConfig.get[:bool]("chalk_debug") and len(dirs) + len(files) != 0:
    error("Due to --debug flag, skipping cleanup; moving the " &
          "following to ./chalk-tmp:")
    for item in files & dirs:
      error(item)

  if chalkConfig.get[:bool]("report_total_time"):
...
proc setupManagedTemp*() =
  let customTmpDirOpt = chalkConfig.getOpt[:string]("default_tmp_dir")
...
proc autocompleteFileCheck*() =
  if isatty(0) == 0 or chalkConfig.get[:bool]("install_completion_script") == false:
    return
...
template otherSetupTasks*() =
  ...
  if isatty(1) == 0:
    setShowColor(false)
  limitFDCacheSize(chalkConfig.get[:int]("cache_fd_limit"))
