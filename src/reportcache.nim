...
template tracePublish(topic, m: string, prevSuccesses = false) =
  ...
  if startSubscriptions == 0 and prevSuccesses:
    ...
    if n == 0 and startSubscriptions != 0:
      if chalkConfig.get[:bool]("use_report_cache"):
        ...
        if chalkConfig.get[:bool]("force_output_on_reporting_fails"):
...
proc loadReportCache(fname: string) =
  once:
    try:
      let
        retries = chalkConfig.get[:int]("report_cache_lock_timeout_sec")
...
proc safePublish*(topic, msg: string) =
  if not chalkConfig.get[:bool]("use_report_cache"):
  ...
  let fname = resolvePath(chalkConfig.get[:string]("report_cache_location"))
