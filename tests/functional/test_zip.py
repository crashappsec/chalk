...
def test_jar_files_never_injected(
    ...
    expected_injection = should_inject_binary(test_file, virtual)
...
@pytest.mark.parametrize("virtual", [True, False])
def test_multi_suffix_files_configured_injection(
    tmp_data_dir: Path,
    chalk: Chalk,
    copy_files: list[Path],
    virtual: bool,
):
    """Test that files with multiple suffixes get binary injected when configured."""
    test_file = copy_files[0]

    # Test with serverless config that allows "serverless.zip" extension
    insert = chalk.insert(
        config=SERVERLESS_CONFIG,
        artifact=test_file,
        virtual=virtual,
    ...
    # Multi-suffix files like "test.serverless.zip" should have binary injected when configured
    expected_injection = should_inject_binary(test_file, virtual)
    verify_binary_injection(test_file, virtual, expected_injection)
